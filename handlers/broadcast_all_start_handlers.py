...
from config import callback_query, callback_message, broadcast_all_state, API_ID, API_HASH, scheduler, Query, bot, conn, \
    New_Message
from func.func import gid_key, create_broadcast_data, get_active_broadcast_groups, get_entity_by_id
...
async def stop_broadcast_all(event: callback_query) -> None:
    ...
    # Отримуємо інформацію про групи для відображення назв замість ID
    ...
    # Перевіряємо, чи є активні розсилки
    has_stopped = False
    
    # Отримуємо клієнта для отримання назв груп
            ...
            try:
                ...
                try:
                    ...
                    if group_username.startswith('@'):
                        ...
                        try:
                            ...
                            if not entity:
                                ...
                                # Пропускаємо подальшу обробку цієї групи
                                continue
                        except ValueError:
                    ...
                    if "Cannot find any entity corresponding to" in str(entity_error):
                        ...
                        logging.error(f"Ошибка при получении информации о группе: {str(entity_error)}")
                        continue
                ...
                try:
                    ...
                    logging.error(f"Критическая ошибка при остановке рассылки: {stop_error}")
                    continue
    
    finally:
        await client.disconnect()
    
    # Якщо немає зупинених розсилок
    if not has_stopped:
...
async def schedule_account_broadcast(user_id: int,
    ...
    # Спочатку видаляємо всі існуючі завдання для цього користувача
    # щоб уникнути дублювання повідомлень
    for job in scheduler.get_jobs():
        if job.id.startswith(f"broadcastALL_{user_id}_"):
            scheduler.remove_job(job.id)
            logging.info(f"Видалено існуюче завдання {job.id}")
    ...
    for group in groups:
        try:
            # Спробуємо отримати об'єкт за username або ID
            ...
            if group_username.startswith('@'):
                ...
                try:
                    ...
                    logging.error(f"Не удалось получить entity по username {group_username}: {e}")
                    continue
            else:
                ...
                try:
                    ...
                    if not ent:
                        logging.error(f"Не удалось получить entity для ID {group_username}")
                        continue
                except ValueError:
                    ...
                    try:
                        ...
                        logging.error(f"Не удалось получить entity для {group_username}: {e}")
                        continue
    ...
    for ent in ok_entities:
                        ...
                        try:
                            if photo_to_send:
                                try:
                                    ...
                                    logging.info(f"Отправлено сообщение с фото в {entity.title}")
                                except Exception as photo_error:
                                    logging.error(f"Ошибка при отправке с фото: {photo_error}")
                                    # Если не удалось отправить с фото, пробуем отправить только текст
                                    await client.send_message(entity, txt)
                                    logging.info(f"Отправлено сообщение без фото в {entity.title}")
                            else:
                                # Отправляем обычное текстовое сообщение
                                await client.send_message(entity, txt)
                                logging.info(f"Отправлено: {txt} в {entity.title}")
                        except Exception as entity_error:
                            # Проверяем, не связана ли ошибка с невозможностью найти entity
                            if "Cannot find any entity corresponding to" in str(entity_error):
                                logging.info(f"Пробуем получить entity другим способом для {entity.id}")
                                ...
                                if new_entity:
                                    if photo_to_send:
                                        try:
                                            ...
                                            logging.info(f"Отправлено сообщение с фото через альтернативный метод в {new_entity.title}")
                                        except Exception as alt_photo_error:
                                            logging.error(f"Ошибка при отправке с фото через альтернативный метод: {alt_photo_error}")
                                            # Если не удалось отправить с фото, пробуем отправить только текст
                                            await client.send_message(new_entity, txt)
                                            logging.info(f"Отправлено сообщение без фото через альтернативный метод в {new_entity.title}")
                                    else:
                                        await client.send_message(new_entity, txt)
                                        logging.info(f"Отправлено через альтернативный метод в {new_entity.title}")
        ...
        logging.info(f"Створено нове завдання {job_id} для групи {ent.title} (@{getattr(ent, 'username', 'без username')})")
        current_time += sec_run
