...
from func.func import gid_key, get_entity_by_id, create_broadcast_data
...
async def send_broadcast_message(user_id: int, group_id: int, text: str, session_string: str, photo_url: Optional[str] = None, max_retries: int = 10) -> None:
    ...
    while retry_count < max_retries:
        try:
                    ...
                    try:
                        ...
                        if photo_to_send:
                            try:
                                ...
                                logging.debug(f"Отправлено сообщение с фото в {getattr(group_entity, 'title', 'группу')}")
                                ...
                                logging.debug(f"Отправлено сообщение без фото в {getattr(group_entity, 'title', 'группу')}")
                        else:
                            # Отправляем обычное текстовое сообщение
                            await client.send_message(group_entity, txt)
                            logging.debug(f"Успешно отправлено в {getattr(group_entity, 'title', 'группу')}")
                        ...
                        if "Cannot find any entity corresponding to" in str(entity_error):
                            logging.error(f"Не удалось найти группу: {entity_error}")
            ...
            logging.error(f"Ошибка при отправке: {type(e).__name__}: {e}")
            retry_count += 1
            await asyncio.sleep(5)
    
    # Если достигли максимального количества попыток
    logging.warning(f"Не удалось отправить сообщение после {max_retries} попыток")
    with conn:
        cursor = conn.cursor()
...
async def stop_broadcast(event: callback_query) -> None:
    ...
    try:
            ...
            if "Cannot find any entity corresponding to" in str(entity_error):
                try:
                    ...
                    try:
                        ...
                        logging.error(f"[DEBUG] Помилка при альтернативному отриманні Entity: {alt_error}")
                        return
                except Exception as alt_error:
                    logging.error(f"[DEBUG] Помилка при альтернативному отриманні Entity: {alt_error}")
                    return
        ...
        logging.error(f"Ошибка при остановке рассылки: {e}")
