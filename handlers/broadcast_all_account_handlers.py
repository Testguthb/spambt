    ...
    with conn:
        ...
        try:
            ...
            for user_id, session_string in sessions:
                # Перевіряємо, чи вже обробляли цей акаунт
                ...
                async with TelegramClient(StringSession(session_string), API_ID, API_HASH) as client:
                    try:
                        ...
                        # Отримуємо активні групи для цього акаунта
                        active_groups = get_active_broadcast_groups(user_id)
                        
                        if not active_groups:
                            continue
                        
                        # Додаємо інформацію про акаунт тільки якщо є активні групи
                        ...
                        # Обробляємо кожну активну групу
                        for group_id in active_groups:
                            ...
                            try:
                                    ...
                                    if group_username.startswith('@'):
                                        ...
                                        try:
                                            ...
                                            if not group_entity:
                                                ...
                                                # Перевіряємо та зупиняємо задачі
                                                job_stopped = False
                                                ...
                                                # Додаємо повідомлення про результат
                                                if job_stopped:
                                ...
                                if isinstance(group_entity, Channel) and group_entity.broadcast and not group_entity.megagroup:
                                    logging.info(f"Пропускаємо канал {group_username}")
                                    continue
                                ...
                                logging.error(f"Ошибка при обработке группы {group_id}: {str(e)}")
                        ...
                        # Додаємо повідомлення про акаунт тільки якщо були зупинені задачі
                        if has_stopped_jobs:
            ...
            # Якщо немає повідомлень про зупинені розсилки, додаємо інформаційне повідомлення
            if len(msg_lines) == 1:  # Тільки заголовок
                                            ...
                                            try:
                                                if photo_to_send:
                                                    try:
                                                        ...
                                                        logging.debug(f"Отправлено сообщение с фото в {entity.title}")
                                                        ...
                                                        logging.debug(f"Отправлено сообщение без фото в {entity.title}")
                                                else:
                                                    # Отправляем обычное текстовое сообщение
                                                    await client.send_message(entity, txt)
                                                    logging.debug(f"Успешно отправлено в {entity.title}")
                                            except Exception as entity_error:
                                                    ...
                                                    if new_entity:
                                                        if photo_to_send:
                                                            try:
                                                                ...
                                                                logging.debug(f"Отправлено сообщение с фото через альтернативный метод в {new_entity.title}")
                                                                ...
                                                                logging.debug(f"Отправлено сообщение без фото через альтернативный метод в {new_entity.title}")
                                                        else:
                                                            await client.send_message(new_entity, txt)
                                                            logging.debug(f"Отправлено через альтернативный метод в {new_entity.title}")
